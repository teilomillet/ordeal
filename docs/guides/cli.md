...
for machine output.
`--replay-attempts N` controls how many exact immediate replays support a
runtime mismatch.
...
modules, classifies changes, saves unexpected divergences, mutates the resulting
tests, and scans the candidate independently. Put explicit domain rules in
...
| `--save-artifacts` | not used | promote a replay-backed failure into the durable manifest |
