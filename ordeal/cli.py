...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    def _render_function_gap_stub(result: Any, item: Any) -> str:
        ...
        if evidence:
            for entry in evidence[:5]:
                lines.append(
                    f"# - {entry.get('kind', 'evidence')}: {entry.get('detail', '')}".rstrip()
                )
        else:
        ...
        for result in audit_results:
            ...
            for item in getattr(result, "function_audits", []):
                ...
                if not target_name or target_name in written_targets:
                    continue
                content = _render_function_gap_stub(result, item).strip()
                path = _gap_stub_path(output_dir, target_name)
                path.parent.mkdir(parents=True, exist_ok=True)
                existing = path.read_text(encoding="utf-8") if path.exists() else None
                if existing != content + "\n":
