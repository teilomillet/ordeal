...
def generate_starter_tests(target: str, *, dry_run: bool = False) -> str:
    ...
    hints only — **no functions are executed**.  This prevents side
    effects when previewing what ``ordeal init --dry-run`` would create.
    ...
    if dry_run:
        return _starter_static(target, is_func=is_func)
    if is_func:
...
def _starter_static(target: str, *, is_func: bool) -> str:
    """Generate test stubs from signatures only — no function execution.

    Used by ``--dry-run`` to preview test structure without side effects.
    Produces one test per callable with typed example arguments and a
    placeholder assertion.  No mining, no fuzzing, no function calls.
    """
    if is_func:
        parts = target.rsplit(".", 1)
        if len(parts) < 2:
        ...
        sig_str, call_args = _resolve_signature(target)

        lines = [
            f'"""Tests for {target} — generated by ordeal init (dry-run).',
            "",
            "Stub tests from function signatures. Run without --dry-run for",
            "full discovery with pinned values and mined properties.",
            '"""',
            "",
            f"from {module_path} import {func_name}",
            "",
    ...
    # Module-level: inspect all public callables
    try:
    ...
    for name in callables:
        ...
        sig_str, call_args = _resolve_signature(dotted)
        lines.append("")
        lines.append(f"def test_{safe_mod}_{name}():")
        lines.append(f'    """{name}{sig_str}"""')
        lines.append(f"    result = {target}.{name}({call_args})")
