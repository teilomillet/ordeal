...
class MutationResult:
    ...
    def generate_test_stubs(self) -> str:
        """Generate a Python test file skeleton for surviving mutants.

        Each surviving mutant produces a test stub with:

        - The mutant's operator, description, and source line as context.
        - The remediation guidance as a docstring.
        - A placeholder body for the developer (or AI) to fill in.
        ...
        module_path = self.target.rsplit(".", 1)[0]
        func_name = self.target.rsplit(".", 1)[-1]
        ...
        for i, m in enumerate(self.survived, 1):
            ...
            lines.append('    """')
            lines.append(f"    # TODO: add assertion that catches {m.description}")
