...
class ChaosTest(RuleBasedStateMachine):
    ...
    def clone_checkpoint_state(self) -> dict[str, Any]:
        ...
        """
        legacy_snapshot_filter = getattr(self, "snapshot_filter", None)
        state: dict[str, Any] = {}
        for name, value in self.__dict__.items():
            if name == "_faults" or not self.checkpoint_snapshot_filter(name, value):
                continue
            if callable(legacy_snapshot_filter) and not legacy_snapshot_filter(name, value):
                continue
            try:
